function createBartChart2(data, yAxis, xAxis, svg, styling) {
  const { width, height, marginTop, marginRight, marginBottom, marginLeft } =
    styling;
  const xScale = d3
    .scaleBand()
    .domain(data.map((d) => d.total_diseases)) // Assuming 'name' is the property for x-axis categories
  ...
  const bars = svg
    ...
    .enter()
    .append("rect")
    .attr("x", (d) => xScale(d.total_diseases))
    ...
    .on("end", function (d, i) {
      ...
      bar
        .append("text")
        .text((d) => d[yAxis])
        .attr("x", xScale(d.total_diseases) + xScale.bandwidth() / 8)
